...
local function GetMinimapAnchor() --https://forum.cfx.re/t/release-utility-minimap-anchor-script/81912
    ...
    Minimap.left_x, Minimap.top_y = GetScriptGfxPosition(minimap_main.posX, (minimap_main.posY + (-minimap_main.sizeY))* (isCircleMode and 1.33 or 1.00))
    ResetScriptGfxAlign()
...
function GetHudDimensionsByMinimapAnchor(inputWidth,inputHeight)
...
AddEventHandler('nbk_circle:RequestHudDimensionsFromMyUI', function(inputWidth,inputHeight,cb,offsetx,offsety)
    ...
    local hud = GetHudDimensionsByMinimapAnchor(inputWidth,inputHeight)
    hud.x = hud.x + offsetx
    hud.y = hud.y + offsety
    cb(hud)
end)
